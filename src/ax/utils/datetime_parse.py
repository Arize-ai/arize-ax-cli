...
def parse_optional_iso8601(value: str | None) -> datetime | None:
    """Parse an optional ISO 8601 datetime string, assuming UTC when naive.

    A value with no offset is stamped as UTC; an explicit offset is preserved.

    Args:
        value: ISO 8601 datetime string, or ``None``.

    Returns:
        A timezone-aware :class:`datetime`, or ``None`` if *value* is ``None``.
